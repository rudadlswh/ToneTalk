...
async function handlePost(request: Request) {
  ...
  try {
    ...
    if (error instanceof SyntaxError || error instanceof ZodError) return jsonError(requestId, 400, "INVALID_INPUT", "언어와 가사를 확인해 주세요. 한 번에 최대 10줄, 줄당 200자까지 번역할 수 있어요.");
