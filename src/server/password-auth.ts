...
export async function passwordAuth(request: Request, signup: boolean) {
  ...
  try {
    ...
    if (error) {
      ...
      console.warn("password_auth_rejected", { requestId, signup, code: error.code, status: error.status });
    ...
    if (!data.session) return jsonError(requestId, 503, "AUTH_SETUP_REQUIRED", "관리자가 Supabase Confirm email 설정을 꺼야 가입할 수 있습니다.");
    return Response.json({ authenticated: true }, { headers: { "Cache-Control": "private, no-store" } });
  } catch { return jsonError(requestId, 503, "AUTH_UNAVAILABLE", "인증 서버에 연결하지 못했습니다. 잠시 후 다시 시도해 주세요.", true); }
}
