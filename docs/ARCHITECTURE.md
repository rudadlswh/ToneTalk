...
현재 Gemini/Ollama 생성 요청은 `postOllama`의 공통 `withAiUsage`를 거친다. 캐시 조회 이후 검증된 Auth UUID의 분/일 횟수와 전체 횟수를 같은 DB 트랜잭션에서 확인하고 동시 전송 슬롯을 획득한다. AI 네트워크 대기 중 DB 트랜잭션을 잡지 않는다. 공급자 장애·429 대기 상태는 같은 범위의 인스턴스가 공유한다. `/api/ai/usage`는 내 앱 사용량만 조회하며 프로필에서 표시한다. 구체적인 제한·데이터/환경 분리·배포 절차는 [AI-USAGE.md](AI-USAGE.md)를 따른다.
...
오늘/저장 문장은 `/api/study/daily`에서 서버 문제로 채점하고 답안과 `study_point_events`를 원자적으로 저장한다. 롤플레이는 서버 발급 세션과 이전 응답 해시를 검증한 뒤 4번째 응답 확정과 XP를 원자적으로 기록한다. 대화 본문은 DB에 저장하지 않는다. `/api/study/points` POST는 이전 원장의 동일 요청 확인만 허용하며 신규 지급은 거절한다. 기존 완료 중복 제약에 더해 `(owner_id,activity,credited_on)` 고유 인덱스로 유형별 한국 날짜당 한 번, 총 80 XP로 제한한다. 한도를 초과해도 학습 완료는 정상 저장한다. 별도 잔액 컬럼 없이 원장 합계를 사용하며 과거 기록과 프로필의 총 XP/커서 기반 적립 기록은 유지한다. [지급 규칙](STUDY-POINTS.md), [검증·API·운영 적용](STUDY-REWARD-PROOF.md)을 참고한다.

### 오답 복습

오답 복습은 `/study/mistakes`에서 기존 문제/오답을 계정 범위로 조회하고 `/api/study/mistakes`에서 서버 재채점한다. `mistake_review_answers`에 별도 제출 이력을 저장해 원래 첫 선택·정답 공개·XP 근거를 보존한다. 복습은 AI 호출/XP 원장 쓰기 없이 처리한다. 학습 유형·어투·최신 복습 상태 필터와 커서 조회를 사용하며 [6단계 문서](MISTAKE-REVIEW.md)를 따른다.
