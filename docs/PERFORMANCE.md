...
이 문서의 최초 측정은 로그인 도입 전이다. 로그인 도입 후 owner 생성은 AsyncLocalStorage 요청 범위에서만 중복 제거하며 API마다 Auth 검증과 초기 프로필 INSERT ON CONFLICT가 추가된다. 이전 응답 시간/SQL 횟수를 현재 인증 포함 성능으로 해석하지 않는다. Profile/Health는 독립 로딩하여 Ollama 장애가 프로필 편집 화면을 막지 않는다. 늦은 기본 설정 응답이 사용자의 언어 선택을 덮어쓰지 않는다.
